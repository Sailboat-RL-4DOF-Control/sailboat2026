...
REWARD_COMPONENTS = ("progress", "action", "checkpoint", "arrival", "time")
REWARD_ABLATIONS = {
    "full": (),
...
def _build_reward_terms(reward_ablation=None, reward_terms=None):
    reward_ablation = reward_ablation or "full"
    if reward_ablation not in REWARD_ABLATIONS:
        valid = ", ".join(sorted(REWARD_ABLATIONS))
        raise ValueError(f"Unknown reward_ablation={reward_ablation!r}. Valid values: {valid}")

    terms = {name: True for name in REWARD_COMPONENTS}
    for name in REWARD_ABLATIONS[reward_ablation]:
        terms[name] = False

    if reward_terms:
        for name, enabled in reward_terms.items():
            if name not in terms:
                valid = ", ".join(REWARD_COMPONENTS)
                raise ValueError(f"Unknown reward component {name!r}. Valid values: {valid}")
            terms[name] = bool(enabled)

    return terms

class SailboatNowindEnv(gym.Env, EzPickle):
    metadata = {
    ...
    def __init__(
        ...
        reward_ablation: Optional[str] = None,
        ...
        # 定义常量
        super().__init__()
        self.reward_ablation = reward_ablation or "full"
        self.reward_terms = _build_reward_terms(self.reward_ablation, reward_terms)
    ...
    def _dryden_wind_update(self, dt):
        """
        使用 Dryden 模型更新风场
        ...
        # 计算风速分量 (基础风 + 湍流扰动)
        # 基础风沿初始风向
        base_wind_x = globals.w_ini_v * np.cos(globals.w_ini_d)
        base_wind_y = globals.w_ini_v * np.sin(globals.w_ini_d)
        ...
        # 将扰动转换到全局坐标系
        cos_d = np.cos(globals.w_ini_d)
        sin_d = np.sin(globals.w_ini_d)
    ...
    def update_wind(self):
        ...
        new_v, new_d = self._dryden_wind_update(dt=1.0)
    ...
    def calculate_reward(self, laststate, truncated):
        ...
        if self.reward_terms["checkpoint"]:
        ...
        if self.reward_terms["arrival"] and dis_to_target <= 10.0:
            reward += 40

        if truncated:
            reward -= 100

        if self.reward_terms["time"]:
            reward -= 0.2
    ...
    def reset(
        ...
        # Use a random direction unless the caller requests a fixed case.
        globals.w0 = [8.0, 0]
        ...
        # The policy sees 14 vessel/target variables and no wind variables.
        self.state = np.zeros(14, dtype=np.float32)

        self.state[12] = 500
        self.state[13] = 0

        # Randomize the initial heading and forward speed.
        self.state[3] = random.uniform(-np.pi, np.pi)
        self.state[4] = random.uniform(0, 1)
