"""Train the 16-dimensional feed-forward SAC policy with wind observations.

Run in a terminal with ``python train_sac_wind.py`` or open this file in an
IDE and click Run. Defaults can be edited in ``build_parser`` below.
"""
...
from SAC_fc import SAC_fc_continuous
from noise_utils_wind import NoiseManager
    ...
    try:
        while total_steps < args.total_training_steps:
            if total_steps < args.warmup_steps:
                ...
                actions = np.asarray(
                    [agent.select_action(states[i], deterministic=False) for i in range(args.num_parallel_envs)]
                )
...
def build_parser():
    ...
    parser.add_argument("--hidden-sizes", default="256,128,64")
...
if __name__ == "__main__":
    ...
    if arguments.output_dir is None:
        arguments.output_dir = str(
            SCRIPT_DIR / "trained_models" / f"sac_wind_seed{arguments.seed}"
        )
    train(arguments)
